...
- 관광공사 `areaBasedSyncList2` 의 `modifiedtime` 은 **"그 날짜 이후" 가 아니라 "그 날 하루"**
  ...
  에 남겨 다음 실행이 이어받습니다. 평상시 호출은 **하루 1~2회**입니다.
...
```json
{ "crons": [ { "path": "/api/cron/sync", "schedule": "0 19 * * *" }] }
```

`0 19 * * *` = 매일 **19:00 UTC = 다음 날 04:00 KST** 입니다.
...
| 실행 시각 | 지정 시각이 든 **1시간 안** 어디선가 (정각 보장 없음) | 새벽이라 영향 없음 |
...
- 한국관광공사 — 국문 관광정보 서비스 / 관광지별 연관관광지 정보 / 두루누비 정보 서비스 / 관광사진 정보
...
**한국관광 데이터랩**은 설계 단계에 저방문 구·군을 고르려고 통계를 한 번 참고했을 뿐,
서비스가 호출하지 않습니다. 상시 출처 표기(`components/DataSources.tsx`)에는 두지 않고
S6 정보 화면의 "설계 단계 참고 자료" 에만 적습니다 — 공공누리 출처표시 의무는 활용한
데이터에 대한 것이라 두 성격을 갈라 적는 편이 정확합니다. `D-7` 시드 발굴에서 실제로
호출하게 되면 그때 상시 표기로 올립니다.
