...
>
> **2026-08-17 — 마이그레이션은 늘지 않았지만 `sync_runs.audit` 에 담기는 것이 늘었습니다.**
> 종전에는 집계표 대조 결과만 들어갔고, 이제는 **자기 열이 없는 실행 건수 셋**(`calls` ·
> `hiddenSeen` · `hidden`)이 함께 들어갑니다 — 모양은
> `{"counts":{…}, "pool":<대조 결과 또는 null>}` 이고 **대조 여부는 `pool` 로 갈립니다.**
> 열을 더하지 않기로 정해진 자리라(`D-66-2`) DDL 변경 0건이며, 위 표에 새 줄이 없는 것이
> 정상입니다. 되짚어 읽을 때 **08-17 이전 행은 옛 모양**(대조 결과가 평평하게 들어 있거나
> `null`)이라는 점만 함께 봅니다.
