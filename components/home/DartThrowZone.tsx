...
export function DartThrowZone({
  ...
  const subCaption = (() => {
    ...
    if (stage === "idle") {
      if (hint) return hint;
      // 조준의 규칙을 한 줄로 — 세게 당기면 멀리(북쪽), 옆으로 눕히면 당긴 반대쪽으로
      // (`D-60-1` 새총. 앞 판본은 "당긴 쪽으로" 였고 동작과 함께 뒤집혔습니다).
      return aimActive
        ? "세게 당길수록 멀리, 옆으로 눕히면 새총처럼 당긴 반대쪽으로"
    ...
    <div className="relative mt-6 flex min-h-60 flex-col items-center">
      ...
      */}
      ...
      <div
        ...
        aria-label={
          aimActive
            ? "다트. 잡아서 당기면 위 지도를 겨눌 수 있습니다. 새총과 같아서, 세게 당길수록 지도 위쪽을 겨누고 옆으로 눕히면 당긴 반대쪽을 겨눕니다. 오른쪽으로 당기면 왼쪽입니다. 놓으면 겨눈 구·군으로 날아갑니다. 키보드에서는 Enter 또는 Space 로 조준 없이 던지며 16개 구·군 중 균등 추첨이 됩니다."
