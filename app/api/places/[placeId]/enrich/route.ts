 ...
 * 뜻이고, 다트 풀 **800곳 남짓**을 한 바퀴 훑으면 캐시가 하나도 막아 주지 못합니다. 그 한
 * 바퀴가 관광공사 개발계정 일 한도(1,000회)를 넘깁니다.
 *
 * 여기에 정확한 건수를 적지 않는 것은 일부러입니다 — 이 논증에 필요한 것은 자릿수뿐인데,
 * 실건수는 크론이 매일 바꿉니다. 실측값의 정본은 `GET /api/pool/stats` 이며 협업 폴더
 * `PROGRESS.md` §전체 현황 표가 그 인용 정본입니다(`M-IMP5-1`).
 *
