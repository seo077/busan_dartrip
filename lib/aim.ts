 ...
 *   · **당긴 쪽으로** — 오른쪽으로 당기면 오른쪽으로 날아갑니다. 좌우는 뒤집지 않습니다.
 *
 * 세로만 뒤집습니다. 아래로 당겨 위로 날리는 것은 던지는 몸짓 그대로지만, 좌우까지 거울처럼
 * 뒤집으면 겨눈 곳과 날아가는 곳이 어긋납니다 — 오른쪽을 겨눴으면 오른쪽으로 가야 합니다.
 *
 ...
 *   가로 = 방향.   똑바로 당기면 가운데, 옆으로 눕힐수록 **당긴 쪽**으로 벌어집니다.
...
export function aimPointFromPull(pull: Point, extent: AimExtent): Point {
  ...
  // 부호를 뒤집지 않습니다 — 오른쪽으로 당기면 오른쪽(+), 왼쪽이면 왼쪽(-)입니다.
  const lateral =
    power === 0 ? 0 : clamp(Math.atan2(pull.x, Math.abs(pull.y) || 1e-6) / (Math.PI / 2), -1, 1);
