 ...
 *   · **당긴 반대쪽으로** — 왼쪽으로 당기면 오른쪽으로 날아갑니다(새총과 같습니다).
 *
 * 이 두 가지를 곱하면 지도의 **어느 자리든** 던질 수 있는 세기 안에서 닿습니다.
 * 당김 벡터를 그대로 뒤집어 쓰지 않고 이렇게 나눈 이유가 여기 있습니다 — 그대로 뒤집으면
 ...
 *   가로 = 방향.   똑바로 당기면 가운데, 옆으로 눕힐수록 **반대쪽**으로 벌어집니다.
...
export function aimPointFromPull(pull: Point, extent: AimExtent): Point {
  ...
  // 당김의 반대 방향이 수직에서 얼마나 기울었는지 (-90°~+90° → -1~+1)
  const lateral =
