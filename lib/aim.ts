...
/*
 * 아래 네 값과 `components/home/BusanMap.tsx` 의 `FIT_PADDING_PX`(28) 는 처음에 **임의로 정한
 * 값**이라 `PROGRESS.md` §발견 이슈 15·18번이 *"실기기에서 만져 보고 정해야"* 로 걸어 두었습니다.
 * **그 조건이 충족됐습니다** — 사용자가 배포본을 직접 던져 본 뒤 *"다트는 괜찮은데 / 새총 잘
 * 적용됐어 좋아"* 로 확인했습니다(`D-61-4` · `D-60-5`, 2026-08-16).
 * **값은 그대로 두고 표식만** 답니다. 짝이 되는 손맛 값 셋은 `lib/gesture.ts` 에 있습니다.
 */

/** 조준점이 상자 가장자리에 붙어 잘리지 않도록 두는 여백(px) — 실사용 확인함 (2026-08-16, `D-61-4`) */
const AIM_INSET_PX = 18;

/** 구·군이 모여 있는 자리 둘레로 두는 여백(px) — 가장자리 구·군도 넉넉히 겨눌 수 있게. 실사용 확인함 (2026-08-16, `D-61-4`) */
const EXTENT_PAD_PX = 16;

/** 조준 범위가 지나치게 좁아지지 않게 두는 최소 크기(px) — 실사용 확인함 (2026-08-16, `D-61-4`) */
...
/** 이 차이 안쪽이면 "경계 근처" 로 알립니다(px) — 실사용 확인함 (2026-08-16, `D-61-4`) */
