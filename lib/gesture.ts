 ...
 *
 * ── 손맛 값에 대하여 (`D-61-4`, 2026-08-16) ─────────────────────────────────
 * 이 파일의 `MAX_PULL_PX`·`MIN_THROW_POWER`·`DEFAULT_POWER` 는 처음에 **임의로 정한 값**이라
 * `PROGRESS.md` §발견 이슈 15·18번이 *"실기기에서 만져 보고 정해야"* 로 걸어 두었습니다.
 * **그 조건이 충족됐습니다** — 사용자가 배포본을 직접 던져 본 뒤 *"다트는 괜찮은데"* 로
 * 확인했습니다. **값은 그대로 두고 「실사용 확인함(2026-08-16)」 표식만** 답니다.
 * 짝이 되는 조준 값 넷은 `lib/aim.ts` 에 같은 표식이 있습니다.
 */
...
/** 당길 수 있는 최대 거리(px). 이보다 더 끌어도 다트는 더 내려오지 않습니다. — 실사용 확인함 (2026-08-16, `D-61-4`) */
export const MAX_PULL_PX = 132;

/** 손을 놓았다고 보기 위한 최소 이동(px). 이보다 작으면 그냥 톡 건드린 것으로 봅니다. */
export const TAP_SLOP_PX = 8;

/** 이 세기 아래로 놓으면 다트가 못 미치고 떨어집니다 — 서버를 부르지 않습니다. — 실사용 확인함 (2026-08-16, `D-61-4`) */
export const MIN_THROW_POWER = 0.34;

/** 드래그를 못 하는 경로(키보드·모션 최소화)가 쓰는 기본 세기. — 실사용 확인함 (2026-08-16, `D-61-4`) */
 ...
 * **상하에는 그 은유를 입히지 않았습니다.**
 *
 * **이것이 확정 사양입니다 (`D-60-5`, 2026-08-16).** `D-60-3` 이 *"먼저 좌우만 하고 써보고
 * 정한다"* 로 걸어 둔 보류는 사용자가 배포본을 직접 던져 본 뒤 *"상하는 그대로 둬"* 로
 * **닫았습니다.** 아래로 당길 때만 발사되게 하는 안은 보류가 아니라 **미채택**이며,
 * 여기는 더 볼 자리가 아닙니다.
 */
